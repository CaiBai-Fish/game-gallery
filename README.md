...
```powershell
.\scripts\package-setup.ps1    # -> dist\GameGallery-1.0.1-setup.exe  （EXE 安装程序，推荐）
.\scripts\package-msi.ps1      # -> dist\GameGallery-1.0.1.msi
.\scripts\package.ps1          # -> dist\GameGallery-1.0.1-portable.exe
```

**推荐用 `GameGallery-1.0.1-setup.exe`**：双击按向导装即可，静默安装用

```bat
GameGallery-1.0.1-setup.exe /SILENT /NORESTART
```

MSI 也可以（适合企业分发）：

```bat
msiexec /i GameGallery-1.0.1.msi /qn
```
...
不想安装的话，用 `GameGallery-1.0.1-portable.exe`：双击运行，免安装、不写注册表。
...
```bat
msiexec /x GameGallery-1.0.1.msi /qn
```
...
.\scripts\package-setup.ps1         # 生成 EXE 安装程序 -> dist\GameGallery-1.0.1-setup.exe（Inno Setup 6）
.\scripts\package-msi.ps1           # 生成 MSI 安装包  -> dist\GameGallery-1.0.1.msi
.\scripts\package.ps1               # 生成免安装单文件 -> dist\GameGallery-1.0.1-portable.exe
```
...
.\scripts\verify-msi.ps1     -MsiPath dist\GameGallery-1.0.1.msi
```
