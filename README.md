...
`scripts/` 下是基于 UI Automation 的端到端测试：真的启动程序、模拟键鼠，并按**像素**和界面状态断言。

```powershell
# 图库主流程（34 项）：首次发现、导航栏筛选、搜索、查看器、右键菜单、
#   删除确认与取消、回收站、以及"删除确认期间库被刷新"的竞态
.\scripts\verify-e2e.ps1      -Exe <exe> -DataDir <临时目录> -TestDir <临时图库目录>

# 显示与交互（14 项）：适应窗口的像素级校验、打开过渡动画、
#   双击/Enter、两处右键菜单、卡死回归（用 CPU 自旋检测）
.\scripts\verify-gallery.ps1  -Exe <exe> -DataDir <临时目录> -TestDir <临时目录>

# 真实截图库压测（19 项）：207 张截图（含 4K PNG 与 jpg），连翻 14 张、缩放、全选
.\scripts\verify-real.ps1     -Exe <exe> -DataDir <临时目录>

# 导航栏图标（4 项）：把"用真实图标"和"强制字体图标"两次渲染做 A/B 像素比对
.\scripts\verify-icons.ps1    -Exe <exe> -DataDir <临时目录>

# MSI（18 项）：静默安装 → 快捷方式 → 系统注册 → 启动 → 静默卸载 → 清理
.\scripts\verify-msi.ps1      -MsiPath dist\GameGallery-1.0.0.msi
```

注意：`verify-e2e.ps1` 会真的把 `-TestDir` 里的文件移到回收站，请只指向临时目录。

几个值得一提的验证手法：

- **适应窗口**不是看 UI 属性，而是用纯色测试图截图，量出图片实际绘制的像素范围是否为 1424×801。
- **共享元素过渡**用窄条高速采样量"图片在某一行的横向范围随时间的变化"：打开时应从缩略图的位置和尺寸连续长大到铺满，关闭时应连续缩回缩略图原位。实测打开时首帧就是缩略图宽度 `210 → 1099 → 1204 → … → 1424`，关闭时 `1424@x8 → 582@x193 → … → 210@x274`（缩略图原位），全程单调、无过冲。
- **卡死回归**读进程累计 CPU 时间：曾经有个无上限的自我重入队导致 UI 线程忙等，卡死时累计烧掉 135 秒 CPU，现在整轮测试的增量在 1 秒以内。
- **导航栏图标**把"用真实图标"和"强制字体图标"两次渲染的同一区域逐像素比对：4 个游戏标签页各有 44–49 个像素不同，而用字体图标的基线是 0。
